...
def tuple_choices_maker(vars_dic):
    ...
    for k, v in vars_dic.items():
        for i in range(1, v["cols"]+1):
            good_key = "col" + str(i)
            try:
                choice_name = v[good_key]["colname"]
                my_key_to_list_of_choices = choice_name.lower() + "_CHOICES" #v[good_key]["choices"]
                my_new_list_of_choices = ALL_MY_CHOICES_GENERAL[my_key_to_list_of_choices]
                
                str_top = f"{k.upper()}_CHOICES = (\n"
                str_mid_list = []
    ...
    my_tuple_choices.append("POLITY_ALTERNATE_RELIGION_GENUS_CHOICES = POLITY_RELIGION_GENUS_CHOICES  \n")
    my_tuple_choices.append("POLITY_ALTERNATE_RELIGION_FAMILY_CHOICES = POLITY_RELIGION_FAMILY_CHOICES  \n")
    my_tuple_choices.append("POLITY_ALTERNATE_RELIGION_CHOICES = POLITY_RELIGION_CHOICES  \n")
...
class {k.capitalize()}Create(PermissionRequiredMixin, CreateView):
    ...
    template_name = "general/{k}/{k}_form.html"
...
class {k.capitalize()}Update(PermissionRequiredMixin, UpdateView):
    model = {k.capitalize()}
    form_class = {k.capitalize()}Form
    template_name = "general/{k}/{k}_update.html"
    permission_required = 'catalog.can_mark_returned'
...
class {k.capitalize()}ListView(generic.ListView):
    model = {k.capitalize()}
    template_name = "general/{k}/{k}_list.html"
    paginate_by = 10
...
class {k.capitalize()}ListViewAll(generic.ListView):
    model = {k.capitalize()}
    template_name = "general/{k}/{k}_list_all.html"
...
class {k.capitalize()}DetailView(generic.DetailView):
    model = {k.capitalize()}
    template_name = "general/{k}/{k}_detail.html"
...
def templates_folder_make(k):
    ...
    rootfolder = "TEMPLATES/all_new_general_html_templates"
...
def general_VARS_MAKER(name_of_section_str, vars_dic):
    """
    ...
    for var_name in vars_dic.keys():
        ...
        if a_list_of_links not in the_big_dic_of_links["General Variables"]["General Variables"]:
            the_big_dic_of_links["General Variables"]["General Variables"].append(a_list_of_links)
